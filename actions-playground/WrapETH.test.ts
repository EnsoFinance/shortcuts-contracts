...
describe.only('Wrap ETH Action', function () {
  ...
  it('should wrap ETH with Portal', async () => {
    ...
    const topUpPortal = await userWithPortalSigner.sendTransaction({
      to: userWithPortal.Portal.address,
      value: BigNumber.from(1),
      gasLimit: 100000,
    });
    await topUpPortal.wait();
    ...
    planner.add(weirolledWETH.deposit().withValue(BigNumber.from(1)));

    const {commands, state} = planner.plan();

    const weirollTx = await userWithPortal.Portal.execute(commands, state, {gasLimit: 100000});
  ...
  it('should wrap ETH with Portal and transfer it to user', async () => {
    ...
    planner.add(weirolledWETH.deposit().withValue(BigNumber.from(1)));
    planner.add(weirolledWETH.transfer(userWithPortal.address, BigNumber.from(1)));
